...
class StockSummary(models.Model):
	...
	state	=	fields.Selection( SC_STATES, string='Status',readonly= True,required=True)
...
# 	}
	...
	def beginning_lines_sn(self, stock_summary_line, sc):
		...
		line_type = "beg"
		self.process_lines_sn(line_type, date, stock_summary_line, sc)
	...
	def process_lines_sn(self,cr, uid,line_type, date,  stock_summary_line, sc):

		##########################################################################
		# fill product
		##########################################################################
		if line_type == "beg":
			self.fill_product_data(stock_summary_line, sc)
			self.update_starting(stock_summary_line, sc)
		return
	...
	def fill_product_data(self,stock_summary_line, sc):
		sql = "select m.product_id,\
					m.product_uom,\
					sum(m.product_uom_qty) \
					from stock_move as m \
					where %s and (m.location_id=%s or m.location_dest_id=%s) \
					and state = 'done' \
					group by product_id,m.product_uom \
					order by m.product_id"

		date = "m.date <= '%s 24:00:00'" % (sc.date_end)
		self.env.cr.execute(sql % (date, sc.location_id.id, sc.location_id.id))
	...
	def update_starting(self, stock_summary_line, sc):
		sql = "select m.product_id,\
	    			m.product_uom,\
	    			m.product_uom_qty, \
	    			m.id \
					from stock_move as m \
	    			where %s and (m.location_dest_id=%s or m.location_id=%s) \
	    			and m.state = 'done' \
	    			order by m.product_id"
		...
		for beg in res:
			...
			sql2 = "update vit_stock_summary_line set \
						qty_start = %s \
						where stock_summary_id=%s and stock_move_id = %s" % \
				   (qty, sc.id, move_id)
	...
	def process_lines_nosn(self,cr, uid,line_type, date,  stock_summary_line, sc):
